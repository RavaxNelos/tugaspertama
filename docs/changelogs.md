...
## [0.0.7] - Resolusi Konflik Git & Pelengkapan .gitignore

### Ditambahkan
- File **`.gitignore`** dilengkapi dengan aturan yang lebih menyeluruh untuk proyek VB.NET/Visual Studio, mencakup:
  - Folder IDE Visual Studio (`.vs/`) dan file user-specific (`*.user`, `*.suo`, `*.userosscache`, `*.sln.docstates`).
  - Hasil build (`[Bb]in/`, `[Oo]bj/`, `[Dd]ebug/`, `[Rr]elease/`, `x64/`, `x86/`, `[Aa]rtifacts/`).
  - Log dan output MSBuild (`*.log`, `[Ll]og/`, `*.binlog`).
  - Hasil test, output publish, dan paket NuGet.
  - File sampah (`.pdb`, `.db`, `.tmp`, `*~`, dsb.).

### Diperbaiki
- **Konflik merge Git yang belum di-resolve** (`<<<<<<< HEAD`, `=======`, `>>>>>>> main`) di beberapa file, yaitu `Form1.vb`, `Form1.Designer.vb`, `Form2.vb`, `Form2.Designer.vb`, `README.md`, dan `.gitignore`, telah dibersihkan dan dikembalikan ke kondisi terbaru yang benar.
- Seluruh marker konflik dihapus sehingga file tidak lagi mengandung kode yang tidak valid dan Visual Studio dapat membuka **designer/GUI** kembali, termasuk fitur drag-and-drop layout.
